	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "SELECT etapa,COUNT(*) as total FROM( SELECT distinct dni,CASE
                    WHEN (fechaNacimiento) = '0000-00-00' || (fechaNacimiento) = ''  THEN 'F: NO REGISTRAN EDAD'
                    WHEN YEAR(CURDATE()) - YEAR(fechaNacimiento)BETWEEN 0 AND 11 THEN 'A: ETAPA NIÑO(0-11 AÑOS)'
                    WHEN  YEAR(CURDATE()) - YEAR(fechaNacimiento)BETWEEN 12 AND 17 THEN 'B: ETAPA ADOLESCENTE (12-17 AÑOS)'
                    WHEN  YEAR(CURDATE()) - YEAR(fechaNacimiento)BETWEEN 18 AND 29 THEN 'C: ETAPA JOVEN(18-29 AÑOS)'
                    WHEN  YEAR(CURDATE()) - YEAR(fechaNacimiento)BETWEEN 30 AND 59 THEN 'D: ETAPA ADULTO(30-59 AÑOS)'
                    ...
                    WHERE  $campo= '$valor' AND per.activo = 'AC' AND fam.activo = 'AC') AS T WHERE etapa<>'' GROUP BY 1 ORDER BY 1";
                    //AND fam.activo = 'AC' 
                    //echo $query;
