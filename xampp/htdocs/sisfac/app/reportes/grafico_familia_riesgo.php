	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "SELECT CASE WHEN puntaje >5 THEN 'A: ALTO RIESGO'  WHEN puntaje >=3 THEN 'B: MEDIANO RIESGO'
                                WHEN puntaje >=0 THEN 'C: BAJO RIESGO' END riesgo, COUNT(*)  total
                                FROM(  SELECT fam.claveGeneral,codigoFicha,SUM(puntaje) as puntaje
                                FROM riesgo rie INNER JOIN familia fam ON rie.idfamilia = fam.idfamilia AND rie.claveGeneral=fam.claveGeneral 
                                WHERE fam.activo='AC' AND nombreFamilia<>'' and $campo = '$valor'  GROUP BY 1,2) AS T  GROUP BY 1";
		...
		<script type="text/javascript">
                    jQuery(function () {
                        jQuery('#container').highcharts({
                            ...
                            title: {
                                text: 'Reporte n\xFAmero de familias en riesgo por etapa de vida',
                                style: {
                                        color: '#FF7474',
                                        fontSize: '30px'
                                }
