	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "SELECT CASE
                            WHEN puntaje >=37 THEN 'A: ALTO RIESGO'
                                    WHEN puntaje >=24 THEN 'B: MEDIANO RIESGO'
                                    WHEN puntaje >=11 THEN 'C: BAJO RIESGO'
                                    END riesgo,COUNT(*) as total
                            FROM(
                            SELECT nombreFamilia,codigoFicha,SUM(puntaje) as puntaje
                            FROM socioeconomico soc INNER JOIN familia fam ON soc.idfamilia = fam.idfamilia AND soc.claveGeneral=fam.claveGeneral
                            WHERE $campo = '$valor' and fam.activo='AC' AND nombreFamilia<>'' GROUP BY 1,2) AS T WHERE puntaje>=11 GROUP BY 1";
		...
		<script type="text/javascript">
                    jQuery(function () {
                        jQuery('#container').highcharts({
                            ...
                            title: {
                                text: 'Reporte n\xFAmero de familia por riesgo socieconómico',
                                style: {
                                        color: '#FF7474',
                                        fontSize: '30px'
                                }
                            ...
                            subtitle: {
                                text: <?php echo "'$_REQUEST[atributo]: $_REQUEST[seleccion] <br> Fecha reporte: ".date('d/m/y H:i:s')."'";?>
                            },
                            xAxis: {
                                type: 'category',
                                labels: {
