	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "
                        SELECT fecha,DATE_FORMAT(fecha,'%Y') ,DATE_FORMAT(fecha,'%m') -1,DATE_FORMAT(fecha,'%d'),descripcion,COUNT(codigoFicha) 
                        total FROM (SELECT DISTINCT STR_TO_DATE(DATE_FORMAT(fam.fechaHistorial,'%Y/%m/%d'),'%Y/%m/%d') as fecha, soc.tipo,soc.descripcion,
                        codigoFicha FROM socioeconomicoH soc INNER JOIN familiaH fam ON soc.idfamiliaH = fam.idfamiliaH AND soc.claveGeneral=fam.claveGeneral 
                        WHERE $campo = '$valor' AND soc.tipo = '$_REQUEST[tipo]' AND soc.descripcion <> '' AND fechaHistorial>='$fechaInicio 23:59:59' AND fechaHistorial<='$fechaFin 23:59:59' 
                        GROUP BY codigoFicha) AS T GROUP BY 1,2,3,4,5 ORDER BY 5,1
                    ";
                    //echo $query;
		...
		<script type="text/javascript">
                    jQuery(function () {
                        jQuery('#container').highcharts({                  
                            ...
                            title: {
                                text: "Reporte tendencia: Número de familias según " + <?php echo "'".strtolower($_REQUEST[tipo])."'";?>
                            },
                            subtitle: {
