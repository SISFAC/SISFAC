	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "SELECT fecha, DATE_FORMAT(fecha,'%Y') ,DATE_FORMAT(fecha,'%m') -1,DATE_FORMAT(fecha,'%d'), opcionDNI, COUNT(*) total, (SELECT p.opcionDNI FROM personaH p WHERE CONCAT(p.nombre,apellidoPaterno,apellidoMaterno) =
                                T.nombre ORDER BY idpersonaH asc LIMIT 1 OFFSET 0)  tipo1, (SELECT CASE WHEN T.opcionDNI = p.opcionDNI THEN 0 ELSE -1 END FROM personaH p 
                                WHERE CONCAT(p.nombre,apellidoPaterno,apellidoMaterno)=T.nombre ORDER BY idpersonaH asc LIMIT 1 OFFSET 0) total2 ,dni
                                FROM ( SELECT DISTINCT sexo, opcionDNI, CONCAT(per.nombre,apellidoPaterno,apellidoMaterno) as nombre,dni,STR_TO_DATE(DATE_FORMAT(fam.fechaHistorial,'%Y/%m/%d'),'%Y/%m/%d') as fecha
                                FROM familiaH fam INNER JOIN personaH per ON fam.idfamiliaH = per.idfamiliaH AND fam.claveGeneral=per.claveGeneral 
                                WHERE  opcionDNI<>'' AND $campo = '$valor' AND per.activo ='AC' AND fechaHistorial>='$fechaInicio 23:59:59' AND fechaHistorial<='$fechaFin 23:59:59' GROUP BY 1,2,3,4,5) AS T GROUP BY 1,2,3,4,5 ORDER BY 1,2";
		...
		<script type="text/javascript">
                    jQuery(function () {
                        jQuery('#container').highcharts({                  
                            ...
                            title: {
                                text: 'Reporte tendencia: Número de personas por tipo de documento'
                            },
                            subtitle: {
                            ...
                            series: [{
                                ...
                                data: [
                                    ...
                                    $suma=0;
                                        while ($row = mysql_fetch_array($result)) {
                                            if($row[4] == 'CARNET DE EXTRANJERIA'){
                                                $suma = $row[5];
                                ...
                                name: "NO TIENE DNI/DNI EN TRAMITE",
                                data: [
                                    
                                    <?php
                                    $suma=0;
                                        while ($row = mysql_fetch_array($result3)) {
                                            if($row[4] == 'NO TIENE DNI/DNI EN TRAMITE'){
                                                $suma = $row[5];
                                                echo "[";?> Date.UTC(<?php echo "$row[1],$row[2],$row[3]" ;?> ) <?php echo ",$suma],";
                                            }
                                        }
