	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "SELECT DISTINCT STR_TO_DATE(DATE_FORMAT(fechaHistorial,'%Y/%m/%d'),'%Y/%m/%d') as fecha, DATE_FORMAT(fechaHistorial,'%Y') ,
                            DATE_FORMAT(fechaHistorial,'%m') -1,DATE_FORMAT(fechaHistorial,'%d'), 
                            CASE WHEN puntaje >=37 THEN 'A: ALTO RIESGO'
                            ...
                            WHERE $campo = '$valor' AND fechaHistorial>='$fechaInicio 00:00:00' AND fechaHistorial<='$fechaFin 23:59:59' AND  fam.activo = 'AC' GROUP BY 1,2,3) AS T
                            WHERE puntaje>=11 GROUP BY 1,2,3,4,5 ORDER BY 1,2";
                    //echo $query;
		...
		<script type="text/javascript">
                    jQuery(function () {
                        jQuery('#container').highcharts({                  
                            ...
                            title: {
                                text: 'Reporte tendencia: Número de familias en riesgo según datos socieconómicos'
                            },
                            subtitle: {
