	...
	<head>
		...
		<style type="text/css">
                ...
                <?php
                    ...
                    $query = "SELECT DISTINCT STR_TO_DATE(DATE_FORMAT(fam.fechaHistorial,'%Y/%m/%d'),'%Y/%m/%d') as fecha,
                                    DATE_FORMAT(fechaHistorial,'%Y') ,
                                    DATE_FORMAT(fechaHistorial,'%m') -1,DATE_FORMAT(fechaHistorial,'%d'), 
                                ent.tipo,  SUM(ent.descripcion) as total
                            FROM familiaH fam INNER JOIN entornoH ent ON fam.idfamiliaH=ent.idfamiliaH AND fam.claveGeneral=ent.claveGeneral 
                            WHERE $campo = '$valor' AND ent.tipo = 'NUMERO DE CANES'
                            AND ent.descripcion<>'' AND fechaHistorial>='$fechaInicio 00:00:00' AND fechaHistorial<='$fechaFin 23:59:59' 
                            AND  fam.idfamiliaH = sf_maxfam(fam.codigoFicha,'$fechaFin') GROUP BY 1,2,3,4,5 ORDER BY 1";
                    //echo $query;
		...
		<script type="text/javascript">
                    jQuery(function () {
                        jQuery('#container').highcharts({                  
                            ...
                            title: {
                                text: 'Reporte tendencia: Número de canes por familia'
                            },
                            subtitle: {
                            ...
                            tooltip: {
                                ...
                                pointFormat: '{point.x:%e. %b}: {point.y:.f} canes(s)'
                            },
                            ...
                            series: [{
                                name: "NUMERO DE CANES",
                                data: [
                                    
                                    <?php
                                    $suma=0;
                                        while ($row = mysql_fetch_array($result1)) {
                                            if($row[4] == 'NUMERO DE CANES'){
                                                $suma = $row[5];
