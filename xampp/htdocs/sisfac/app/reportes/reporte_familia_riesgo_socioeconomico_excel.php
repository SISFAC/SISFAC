...
$excel = $archivo->load('Reporte_familia_riesgo_socioeconomico_excel.xlsx');
...
//while($rowgen = mysql_fetch_array($resultgen)){
    $query1 = "
        ...
        END riesgo,COUNT(*)
        FROM(
        SELECT DISTINCT nombreFamilia,codigoFicha,SUM(puntaje) as puntaje
        FROM socioeconomico soc INNER JOIN familia fam ON soc.idfamilia = fam.idfamilia AND soc.claveGeneral=fam.claveGeneral 
        WHERE  nombreFamilia<>'' and fam.activo='AC' $wh  GROUP BY 1,2) AS T WHERE puntaje>=11 GROUP BY 1 
        ";
