...
$excel = $archivo->load('Reporte_personas_visita.xlsx');
global $wh;
...
while($rowgen = mysql_fetch_array($resultgen)){
    $query1 = "SELECT codigoFicha,nombreFamilia, resultado, COUNT(*) 
                FROM familia fam INNER JOIN visita vis ON vis.idfamilia = fam.idfamilia AND vis.claveGeneral=fam.claveGeneral
                WHERE fam.activo='AC' $wh GROUP BY 1,2,3 ORDER BY 1";
