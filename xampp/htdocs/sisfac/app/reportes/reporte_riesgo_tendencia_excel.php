...
$excel = $archivo->load('RepTendencia_FamiliasRiesgoEtapaVida.xlsx');
...
while($rowgen = mysql_fetch_array($resultgen)){
    $query1 = "
        SELECT DISTINCT STR_TO_DATE(DATE_FORMAT(fechaHistorial,'%Y/%m/%d'),'%Y/%m/%d') as fecha, CASE WHEN puntaje >5 THEN 'A: ALTO RIESGO'
        ...
        WHERE fam.activo = 'AC' $wh AND fechaHistorial>='$fechaInicio 00:00:00'  AND fechaHistorial<='$fechaFin 23:59:59' GROUP BY 1,2,3) AS T  GROUP BY 1,2
        ";
