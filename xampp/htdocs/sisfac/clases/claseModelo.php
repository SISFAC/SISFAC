...
class Nombre {
    ...
    public function mostrarDatagrid($_REQUEST){
        $limit = $_REQUEST['rows'];
        $page = $_REQUEST['page'];
        $sidx = $_REQUEST['sidx'];
        $sord = $_REQUEST['sord'];
        ...
        $query="SELECT campo FROM tabla WHERE 1=1 $wh ORDER BY $sidx $sord LIMIT $start,$limit";
        //echo $query;
    ...
    public function agregar(){
        $data = verificarDatos('add', array('campo'=>$campo));
        if($data[0]!=''){
    ...
    public function actualizar($id){
        $data = verificarDatos('edit', array('campo'=>$campo)); 
        if($data[0]!=''){
            $query = "UPDATE tabla SET $data[0] WHERE id = $id AND claveGeneral = '$claveGeneral'";
            mysql_query($query);
            echo $query;
        }
    }
