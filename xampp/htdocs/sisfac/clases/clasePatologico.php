...
class AntecedentePatologico {
    ...
    public function mostrarAntecedentePatologicoDatagrid(){
        $limit = $_REQUEST['rows'];
        $page = $_REQUEST['page'];
        $sidx = $_REQUEST['sidx'];
        $sord = $_REQUEST['sord'];
        ...
        $query="SELECT COUNT(*) FROM antecedentePatologico WHERE 1=1 $wh ";
        //echo $query;
        $dato = obtenerPaginacion($query, $limit, $page);
        $start = $dato[0];$count = $dato[1];$total_pages=$dato[2];
        $query="SELECT idantecedentePatologico, claveGeneral, idpersona, tipo, fecha, idcatalogoCIE10, nombreCatalogo, fuente, observacion FROM antecedentePatologico WHERE 1=1 $wh ORDER BY $sidx $sord LIMIT $start,$limit";
        //echo $query;
    ...
    public function actualizar($id){
        ...
        if($data[0]!=''){
            $query = "UPDATE antecedentePatologico SET $data[0] WHERE id = $id";
            mysql_query($query);
            echo $query;
        }
    }
    ...
    public function eliminar($id){
        $query = "DELETE FROM antecedentePatologico WHERE idantecedentePatologico = $id";
        mysql_query($query);
    }
}
?>